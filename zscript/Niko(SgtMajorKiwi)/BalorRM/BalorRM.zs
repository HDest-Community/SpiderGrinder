...
class BalorRM:Balor{
	...
	missile:
		---- A 0 A_StartSound("baby/walk");
		BSPI AB 5 A_TurnToAim(10,targetheight:(frandom(0,target.height*0.6)));
		---- A 0 A_StartSound("baby/walk");
//		---- A 0 A_JumpIf(!HDMobAI.TryShoot(self,flags:hdmobai.TS_GEOMETRYOK),"see");
		BSPI CD 5 A_TurnToAim(10,targetheight:(frandom(0,target.height*0.6)));
		---- A 0 A_StartSound("baby/walk");
		BSPI EF 5 A_TurnToAim(10,targetheight:(frandom(0,target.height*0.6)));
		loop;
	shoot:
		BSPI A 1 A_StartAim(mintics:10,maxtics:35,dontlead:true);
		BSPI A 8{
		...
		BSPI GGHH 1 bright light("PLAZMABX2")A_StartSound("weapons/plasidle",CHAN_WEAPON);
	shootpb2:
		BSPI HGGHGHHG 2 bright light("PLAZMABX2"){
			A_ThunderZap();
			if(target&&distance3d(target)>random(0,150))
				A_FaceTarget(4*(distance3d(target)/150),4*(distance3d(target)/150));
				if(!alt)alt=random(0,1);
				//4*(distance3d(target)/150)
		}
		---- A 0 A_Watch();
